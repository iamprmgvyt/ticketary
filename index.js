...
const { t, emojis } = db;
const registerEvents = require('./botEvents');

// Environment configurations
const PORT = process.env.PORT || 3000;
const ACCESS_SECRET_KEY = process.env.TRANSCRIPT_ACCESS_SECRET;
...
// Serve Favicon Logo
app.get('/ticketary.png', (req, res) => {
    const logoPath = path.join(__dirname, 'ticketary.png');
...
// Serve Emojis folder statically (allowing retrieval of custom PNG emojis from web host)
app.use('/emojis', express.static(path.join(__dirname, 'emoji_pack')));

// Security Middleware checking X-Access-Secret header (for Vercel OAuth API)
const secureMiddleware = (req, res, next) => {
    const secretHeader = req.header('X-Access-Secret');
    
    if (!ACCESS_SECRET_KEY) {
        console.error("❌ SECURITY ALERT: TRANSCRIPT_ACCESS_SECRET environment variable is not defined!");
        return res.status(500).send({ error: 'Server configuration error.' });
    }

    if (secretHeader !== ACCESS_SECRET_KEY) {
        console.warn(`🚨 SECURITY VIOLATION: Denied access to unauthorized client at IP ${req.ip}`);
        return res.status(401).send({ error: 'Unauthorized Access: Invalid Secret Key.' });
    }
...
app.get('/', async (req, res) => {
    let activeGuilds = 0;
    try {
        activeGuilds = client.ws.status === 0 ? client.guilds.cache.size : 1; 
    } catch(e) {}
        ...
        <head>
            ...
            <style>
                ...
                * {
                ...
                /* Real Statistics Cards with float animations */
                .stats-container {
                    display: grid;
...
// Route 2: Upgraded Transcript Viewer (GET /transcript/:id) WITHOUT MFA Login Wall (loaded from MySQL)
...
// Auto-clean Scheduler (Runs every hour to remove transcripts older than 1 week / 7 days from MySQL)
setInterval(async () => {
    try {
        ...
        if (result.affectedRows > 0) {
            console.log(`🧹 Auto-clean: Deleted ${result.affectedRows} expired transcript records (older than 7 days)`);
        }
    } catch (error) {
        console.error('❌ Auto-clean: Interval loop error:', error.message);
    }
}, 60 * 60 * 1000); // 1 hour interval

// --- Hot-Reload Watcher for botCommands.js ---
const commandsPath = path.resolve(__dirname, 'botCommands.js');
fs.watch(commandsPath, (eventType) => {
    if (eventType === 'change') {
        try {
            // Delete cache so the next dynamic require() loads the modified file instantly
            delete require.cache[require.resolve('./botCommands')];
            console.log('🔄 Hot-Reload: Clear cache for botCommands.js successfully.');
        } catch (err) {
            console.error('❌ Hot-Reload: Error clearing require cache:', err.message);
        }
    }
});
...
// Start Web Server
app.listen(PORT, () => {
    console.log(`✅ Express web server listening on port ${PORT}. Ready to host transcripts.`);
});
...
// Bind client to database for Proxy-based emoji cache lookup
db.client = client;

// Load and bind command/event handlers
